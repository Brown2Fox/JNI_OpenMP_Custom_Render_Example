...
template<size_t DIM, typename T, typename U>
vec<DIM, T> operator-(vec<DIM, T> lhs, vec<DIM, U> rhs)
{
    for (size_t i = DIM; i--; )
    {
        lhs[i] -= rhs[i];
 ...
 * @param lhs - vector itself
 * @param rhs - scalar itself
 * @return vector
 */
template<size_t DIM, typename T, typename U>
...
template<typename T>
vec<3, T> operator^ (vec<3, T> lhs, vec<3, T> rhs)
{
...
typedef mat<4, 4, float> Matrix4x4f;
